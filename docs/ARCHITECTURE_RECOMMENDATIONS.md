...
| [`+page.svelte`](file:///Users/sarathsadasivanpillai/projects/herderweb/web/src/routes/pane/%5Bid%5D/+page.svelte) | Core pane view: raw terminal scrollback (no chat/transcript view), auto-scrolling, and an agent-only direct-control swipe toggle. | Includes bottom-pinned scroll tracking to prevent jumping while reading historical logs. Operates cleanly in terminal scrollback mode. |
